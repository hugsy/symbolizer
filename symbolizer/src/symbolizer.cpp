...
template<>
struct fmt::formatter<NumberHuman_t> : fmt::formatter<std::string>
{
    template<typename FormatContext>
    auto
    format(const NumberHuman_t& Number, FormatContext& Ctx) const -> decltype(Ctx.out())
    {
...
struct fmt::formatter<SecondsHuman_t> : fmt::formatter<std::string>
{
    template<typename FormatContext>
    auto
    format(const SecondsHuman_t& Micro, FormatContext& Ctx) const -> decltype(Ctx.out())
    {
        return fmt::format_to(Ctx.out(), "{:.1f}{}", Micro.Value, Micro.Unit);
    }
};

[[nodiscard]] constexpr NumberHuman_t
NumberToHuman(const uint64_t N_)
{
    const char* Unit = "";
    double N         = double(N_);
...
struct Opts_t
{
    ...
    Symbolizer::TraceStyle_t Style = Symbolizer::TraceStyle_t::FullSymbol;
...
bool
SymbolizeFile(Symbolizer::DbgEng_t& Dbg, const fs::path& Input, const fs::path& Output)
{
    //
    // Open the input trace file.
    //
    const UniqueHandle TraceFile {::CreateFileA(
        Input.string().c_str(),
        GENERIC_READ,
        FILE_SHARE_READ,
        nullptr,
        ...
        nullptr)};
    if ( !TraceFile || TraceFile.get() == INVALID_HANDLE_VALUE )
    {
        fmt::print("Could not open input {}\n", Input.string());
        return false;
    }

    const UniqueHandle Mapping {::CreateFileMappingA(TraceFile.get(), nullptr, PAGE_READONLY, 0, 0, nullptr)};
    if ( !Mapping || Mapping.get() == INVALID_HANDLE_VALUE )
    {
        fmt::print("Could not create a mapping\n");
        return false;
    }

    const UniqueHandle View {::MapViewOfFile(Mapping.get(), FILE_MAP_READ, 0, 0, 0)};
    if ( !View )
    {
    ...
    std::optional<fmt::v10::ostream> Out;
    if ( !OutputIsStdout )
    {
    ...
    for ( uint64_t LineNumber = 0; (LineFeed = strchr(Line, '\n')) != nullptr; LineNumber++ )
    {
        ...
        {
            fmt::print(
                "{}:{}: Symbolization of {} failed ('{}'), skipping\n",
...
main(int argc, char* argv[])
{
    ...
    //
    CLI::App Symbolizer("Symbolizer - A fast execution trace symbolizer for Windows");

    Symbolizer.allow_windows_style_options();
    Symbolizer.set_help_all_flag("--help-all", "Expand all help");

    Symbolizer.add_option("-i,--input", Opts.Input, "Input trace file or directory")
        ->check(CLI::ExistingPath)
        ->required();
    Symbolizer.add_option("-c,--crash-dump", Opts.CrashdumpPath, "Crash-dump path")
        ->check(CLI::ExistingFile)
        ->required();
    Symbolizer.add_option("-o,--output", Opts.Output, "Output trace (default: stdout)");
    Symbolizer.add_option("-s,--skip", Opts.Skip, "Skip a number of lines")->default_val(0);
    Symbolizer.add_option("-m,--max", Opts.Max, "Stop after a number of lines")->default_val(20'000'000);

    const std::unordered_map<std::string, Symbolizer::TraceStyle_t> TraceStypeMap = {
        {"modoff", Symbolizer::TraceStyle_t::Modoff},
        {"fullsym", Symbolizer::TraceStyle_t::FullSymbol}};

    Symbolizer.add_option("--style", Opts.Style, "Trace style")
        ->transform(CLI::CheckedTransformer(TraceStypeMap, CLI::ignore_case))
        ->default_val("fullsym");
    Symbolizer.add_flag("--overwrite", Opts.Overwrite, "Overwrite the output file if necessary")->default_val(false);
    Symbolizer.add_flag("--line-numbers", Opts.LineNumbers, "Include line numbers")->default_val(false);
    ...
    if ( InputIsDirectory )
    {
        ...
        {
            fmt::print(
                "When the input is a directory, the output can only be either "
    ...
    for ( const auto& Input : Inputs )
    {
        ...
        if ( OutputIsDirectory )
        {
            ...
            Output = Opts.Output / fmt::format("{}.symbolizer", Input.filename().string());
        }
        ...
        Stats.NumberFiles++;
        fmt::print("[{} / {}] {} done\r", Stats.NumberFiles, Inputs.size(), Input.string());
    }
    ...
    fmt::print(
        "Completed symbolization of {} addresses ({} failed) in {} across "
        "{} files.\n",
